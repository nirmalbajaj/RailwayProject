...
<%@page import="Table_Display.Holiday_master_Data" %>
<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList h1 = (ArrayList)request.getSession(false).getAttribute("disp_hol"); %>
...
<title>Holiday Master Data</title>
<style>
/* side nav-bar style */
::after,
::before {
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Holiday Master Data</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>Holiday ID</th>                 <th>Holiday Date</th>             <th>Holiday Description</th>  
      </thead>
        <tbody>
		...
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < h1.size(); rc1++) { %>
				<tr>
					<% Holiday_master_Data h = (Holiday_master_Data) h1.get(rc1); %>
					<td>
						<%= h.getHoliday_id() %>
					</td>
					<td>
