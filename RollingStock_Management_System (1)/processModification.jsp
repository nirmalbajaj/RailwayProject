...
<%@page import="Table_Display.process_modification" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList m1 = (ArrayList)request.getSession(false).getAttribute("disp_pm"); %>
...
<title>Process Modification</title>
<style>
/* side nav-bar style */
::after,
::before {
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Process : Modification</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>Notification Number</th>         <th>RS Owner</th>             <th>RS No</th>   
         <th>RS Type</th>                 <th>Repair Type</th>             <th>Arrival Date</th>
         <th>Enter Stage Details</th>
      </thead>
        <tbody>
		<!-- jsp CODE TO FETCH DATA FROM ROWS-->
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < m1.size(); rc1++) { %>
				<tr>
					<% process_modification m = (process_modification) m1.get(rc1); %>
					<td>
						<%= m.getNotification_no() %>
					</td>
					<td>
						<%= m.getRs_owner() %>
					</td>
