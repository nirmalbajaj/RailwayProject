...
<%@page import="Table_Display.process_inHolding" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList h1 = (ArrayList)request.getSession(false).getAttribute("disp_pih"); %>
...
<title>Process In Holding</title>
<style>
/* side nav-bar style */
::after,
::before {
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Process : In-Holding</caption>
        ...
        <th>Notification Number</th>                 <th>RS Owner</th>             <th>RS Type</th>   
         <th>Repair Type</th>                 <th>Arrival Date</th>             <th>Enter Stage Details</th>
                      
      </thead>
        <tbody>
		<!-- jsp CODE TO FETCH DATA FROM ROWS-->
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < h1.size(); rc1++) { %>
				<tr>
					<% process_inHolding h = (process_inHolding) h1.get(rc1); %>
					<td>
						<%= h.getNotification_no() %>
					</td>
					<td>
						<%= h.getRs_owner() %>
					</td>
