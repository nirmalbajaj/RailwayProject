...
<%@page import="Table_Display.RS_failure" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList f1 = (ArrayList)request.getSession(false).getAttribute("disp_rsf"); %>
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Rolling Stock Failure</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>Coach Number</th>         <th>Coach Category</th>             <th>Coach type</th>   
         <th>Owning railway</th>                 <th>Problem Description</th>             <th>problem Date</th>
         <th>Assembly Description</th>         <th>Sub assembly description</th>             <th>Reporting depot</th>   
         <th>Reported defect</th>                 <th>Work done</th>             <th>Workshop Remarks</th>
         <th>Status</th>         <th>Train No</th>             <th>POH Date</th>   
         <th>Days after POH</th>                 <th>Defect ID</th>             
         
      </thead>
        <tbody>
		<!-- jsp CODE TO FETCH DATA FROM ROWS-->
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < f1.size(); rc1++) { %>
				<tr>
					<% RS_failure f = (RS_failure) f1.get(rc1); %>
					<td>
						<%= f.getCoach_number() %>
					</td>
					<td>
					...
					<td>
						<%= f.getProblem_description() %>
					</td>
					<td>
						<%= f.getProblem_date() %>
					</td>
					<td>
						<%= f.getAssembly_description() %>
					</td>
					<td>
						<%= f.getSub_ass_des() %>
					</td>
					<td>
						<%= f.getReporting_depot() %>
					</td>
					<td>
						<%= f.getReported_defect() %>
					</td>
					<td>
						<%= f.getWork_done() %>
					</td>
					<td>
						<%= f.getWorkshop_remarks() %>
					</td>
					<td>
						<%= f.getStatus() %>
					</td>
