...
<%@page import="Table_Display.rs_condemnation" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList c1 = (ArrayList)request.getSession(false).getAttribute("disp_rsc"); %>
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Process : In-Holding</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>Notification Number</th>                 <th>RS ID</th>             <th>RS Type</th>   
         <th>RS No</th>                 <th>RS Owner</th>             <th>Status</th>
          <th>Authority</th> 			 <th>Condemnation Type</th>		 <th>Remarks</th>
		...
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < c1.size(); rc1++) { %>
				<tr>
					<% rs_condemnation c = (rs_condemnation) c1.get(rc1); %>
					<td>
						<%= c.getNotification_no() %>
					</td>
					<td>
						<%= c.getRs_id() %>
					</td>
					...
					<td>
						<%= c.getCondemnation_type() %>
					</td>
					<td>
						<%= c.getRemarks() %>
					</td>
					<td>
						<%= c.getCondemnation_date() %>
					</td>
