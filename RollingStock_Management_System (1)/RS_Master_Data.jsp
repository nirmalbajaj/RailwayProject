...
<%@page import="Table_Display.RS_Master_Data" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList rs1 = (ArrayList)request.getSession(false).getAttribute("disp_rs"); %>
...
<title>Rolling Stock Master Data</title>
<style>
/* side nav-bar style */
::after,
::before {
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Rolling Stock Master Data</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>RS ID</th>                 <th>RS number</th>             <th>RS Type</th>
        <th>Owning Railway</th>                  <th>Built At</th>          <th>Built Date</th>
	    <th>Start Date</th>              <th>Status</th>               <th>Base Depot</th>   
	    <th>Workshop</th>               <th>Division</th>             <th>Make</th>  
      </thead>
        <tbody>
		<!-- jsp CODE TO FETCH DATA FROM ROWS-->
		<%! int rc; %>	
			<% for(rc = 0; rc < rs1.size(); rc++) { %>
				<tr>
					<% RS_Master_Data m = (RS_Master_Data) rs1.get(rc); %>
					<td>
						<%= m.getRs_id() %>
					</td>
					<td>
