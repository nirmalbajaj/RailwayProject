...
<%@page import="Table_Display.Storage_location_data" %>

<% String ltype = (String)request.getSession(false).getAttribute("ltype"); %>

	<% ArrayList l1 = (ArrayList)request.getSession(false).getAttribute("disp_loc"); %>
...
<title>Storage Location Master Data</title>
<style>
/* side nav-bar style */
::after,
::before {
...
<div class="wrapper">
  ...
  <div class="outer-wrapper" style="padding:8px;">
    ...
    <caption style="font-size:25px;">Location Master Data</caption>
        <thead style="background-color: #CD853F;color:Yellow;">
        <th>Storage Location</th>                 <th>Description</th>             <th>Valid From Date</th>  
      </thead>
        <tbody>
		<!-- jsp CODE TO FETCH DATA FROM ROWS-->
		<%! int rc1; %>	
			<% for(rc1 = 0; rc1 < l1.size(); rc1++) { %>
				<tr>
					<% Storage_location_data l = (Storage_location_data) l1.get(rc1); %>
					<td>
						<%= l.getLocation() %>
					</td>
					<td>
